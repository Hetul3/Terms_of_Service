  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      if (selectedFile.size > maxSize) {
        alert("File is too large. Limit to under 5MB");
        return;
      }
          ...
          {!file ? (
            <div className="w-full py-9 bg-gray-50 rounded-2xl border border-gray-300 gap-3 grid border-dashed">
              <div className="grid gap-1">
                ...
                <h2 className="text-center text-gray-400 text-xs leading-4">PNG, JPG or PDF, smaller than 5MB</h2>
              </div>
            ...
            <div className="w-full py-4 bg-gray-50 rounded-2xl border border-gray-300 flex flex-col items-center">
              <div className="mb-2">
                {preview ? (
                  <img src={preview} alt="Preview" className="max-w-full h-32 object-contain" />
                ) : (
              ...
              <div className="text-center text-gray-900 text-sm font-medium">{file.name}</div>
              <div className="mt-4 flex space-x-4">
                <button
                  type="button"
                  onClick={handleCancel}
                  className="px-4 py-2 bg-gray-200 text-gray-700 rounded-full shadow hover:bg-gray-300 focus:outline-none"
                >
                  Cancel
                </button>
                <button
